...
```{r}
weighted_metrics <- Map(function(list_of_dfs, weights_df) {
  
  lapply(list_of_dfs, function(df) cbind(df, weights_df))
...
```{r}
weighted.quantile(weighted_metrics[[1]]$short$metric_result, probs = 0.5, w = weighted_metrics[[1]]$short$mc_weights)
```
```{r}
weighted.quantile(weighted_metrics[[1]]$short$metric_result, probs = 0.05, w = weighted_metrics[[1]]$short$mc_weights)
```

```{r}
weighted.quantile(weighted_metrics[[1]]$short$metric_result, probs = 0.95, w = weighted_metrics[[1]]$short$mc_weights)
```

```{r}
compute_weighted_quantile <- function(data, weight_type, probs) {
  # Check if the weight type is valid
  if (!(weight_type %in% colnames(data))) {
    stop("Invalid weight type. Please provide a valid column name.")
  }
  
  # Sort the data by metric_result
  sorted_data <- data[order(data$metric_result), ]
  
  # Compute cumulative weights
  cum_weights <- cumsum(sorted_data[[weight_type]])
  
  # Find the index where the cumulative weights exceed the desired quantile
  quantile_index <- which.max(cum_weights >= probs * sum(sorted_data[[weight_type]]))
  
  # Return the metric result at the found index
  return(sorted_data$metric_result[quantile_index])
}
```

```{r}
compute_weighted_quantile(weighted_metrics[[1]]$short, weight_type = "ocean_uptake_weights", probs = 0.95)
```
