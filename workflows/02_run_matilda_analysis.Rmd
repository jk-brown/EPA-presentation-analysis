...
```{r}
# compute RMSE values with GMST and omit values with RMSE < 0.18
ensemble_gmst_rmse <- lapply(result, function(df) {
  
  gmst_data <- subset(df, variable == "gmst")
  
  # calc rmse values
  gmst_rmse <- rmse_values(df, criterion_gmst_obs(), sigma = 1)
  
  # subset the result for gmst rmse < 0.18
  #gmst_rmse_subset <- subset(gmst_rmse, rmse_value < 0.4)
  
  # return gmst_rmse 
  return(gmst_rmse)
})

# merge rmse subset with result to get constrained result
result_rmse_filtered <- Map(function(x, y){
  
  merge(x, y, by = "run_number")
  
}, result, ensemble_gmst_rmse)

#score the constrained result
weights_list <- lapply(result_rmse_filtered, function(result_df){  
  ...
  # creating score_list
  score_list = list(scores_co2, scores_ocean_c)
  
  # multi-criterion weights
  mc_weights = multi_criteria_weighting(score_list)
  
  # data frame of weights
  weights_df = data.frame(run_number = scores_co2$run_number,
...
row.names(weights_df) <- NULL


#### TEST ####
weights_for_merge <- data.frame(weights = weights_list$`SSP1-1.9`$mc_weights)
weighted_params <- params
weighted_params$prob_weight <- weights_for_merge
```
...
```{r}
warming_data <- lapply(result_rmse_filtered, function(df){
...
```{r}
names(result_95_to_14) <- c("SSP1-1.9", "SSP2-4.5", "SSP3-7.0", "SSP5-8.5")
...
```{r}
weighted_metrics <- Map(function(x, y) {
  
  merge(x, y, by = "run_number")
  
}, metric_result, weights_list)
...
```{r}
# function to re-weight scores after filtering out near 0 scores
reweight <- function(df, weight_column = "mc_weights", new_column_name = "reweighted_value"){
  
  # calculate the total weight for unique run numbers
  total_weight <- sum(df[[weight_column]][!duplicated(df$run_number)])
  
  # re-weight current score values
  df[[new_column_name]] <- df[[weight_column]] / total_weight
  
  return(df)
}

# multi-criterion 
metric_summary_mc <- weighted_metrics_df %>%
  reweight("mc_weights") %>% 
...
# CO2 weighted
metric_summary_co2 <- weighted_metrics_df %>%
  reweight("co2_weights") %>% 
  group_by(scenario, metric_criterion) %>%
  summarize("lower_ci" = weighted.quantile(metric_result,
                                           w = co2_weights,
                                           probs = 0.05),
...
metric_summary_ocean_uptake <- weighted_metrics_df %>%
  reweight("ocean_uptake_weights")%>%
  group_by(scenario, metric_criterion) %>%
  summarize("lower_ci" = weighted.quantile(metric_result,
